from codex.models import Snippet
...
class SnippetViewTestCase(APITestCase):
    ...
    def test_list(self):
        response = self.client.get('/codex/snippets/')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data), 2)

    def test_detail(self):
        response = self.client.get('/codex/snippets/{}/'.format(self.s1.id))
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['id'], self.s1.id)

    def test_create(self):
        payload = {'title': 't3', 'code': """print("Create, World.")"""}
        response = self.client.post('/codex/snippets/', payload)
    ...
    def test_update(self):
        ...
        response = self.client.put('/codex/snippets/{}/'.format(self.s1.id), payload)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_partial_update(self):
        payload = {'title': 't666'}
        response = self.client.patch('/codex/snippets/{}/'.format(self.s1.id), payload)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['title'], 't666')

    def test_delete(self):
        response = self.client.delete('/codex/snippets/{}/'.format(self.s1.id))
