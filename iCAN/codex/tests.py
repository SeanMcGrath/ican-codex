...
class SteamViewTestCase(TestCase):
	def testSteam(self):
		resp = self.client.get('/codex/steam/')
		self.assertEqual(resp.status_code, 200)

	def testSteamDiscountedGames(self):
		resp = self.client.get('/codex/steamDiscountedGames/')
		self.assertEqual(resp.status_code, 200)
...
class HackathonViewsTestCase(TestCase):
	def testIndex(self):
		resp = self.client.get('/codex/api/')
		self.assertEqual(resp.status_code, 200)

	def testQuandlDowJones(self):
		resp = self.client.get('/codex/quandlDowJones/')
		self.assertEqual(resp.status_code, 200)

	def testQuandlSnp500(self):
		resp = self.client.get('/codex/quandlSnp500/')
		self.assertEqual(resp.status_code, 200)

	def testQuandlNasdaq(self):
		resp = self.client.get('/codex/quandlNasdaq/')
		self.assertEqual(resp.status_code, 200)

	def testGithubUser(self):
		resp = self.client.get('/codex/githubUser/')
		self.assertEqual(resp.status_code, 200)

	def testGithubTopRepositories(self):
		resp = self.client.get('/codex/githubTopRepositories/')
		self.assertEqual(resp.status_code, 200)

	def testGithubResume(self):
		resp = self.client.get('/codex/githubResume/')
		self.assertEqual(resp.status_code, 200)

	def testNytimespop(self):
		resp = self.client.get('/codex/nytimespop/')
		self.assertEqual(resp.status_code, 200)

class YelpTestCase(TestCase):
	def testYelpPost(self):
		resp = self.client.post('/codex/yelp/', {'location': 'yelp-san-francisco'})
		self.assertEqual(resp.status_code, 200)
	...
	def testGetYelpPage(self):
		resp = self.client.get('/codex/yelp')
...
class TwilioTestCase(TestCase):
	def testTwilio(self):
		resp = self.client.get('/codex/twilio/')
		self.assertEqual(resp.status_code, 200)

	#def testMessage(self):
	#	resp = self.client.post('/codex/twilio/', {'number': '+13473282978', 'message': 'hello world'})
...
class NewYorkTimesTestCase(TestCase):
	...
	def testTopArticles(self):
		resp = self.client.get('/codex/nytimestop/')
		self.assertEqual(resp.status_code, 200)
	...
	def testNewYorkTimesArticles(self):
		resp = self.client.get('/codex/nytimesarticles/')
		self.assertEqual(resp.status_code, 200)
